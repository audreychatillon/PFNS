...
void DrawCut()
{
  ...
  // === =========================== === //
  //TFile* cutfile = new TFile("cut/cut_Q1vsEdiscri_neutron_CHINUlg.root","update");
  //TFile* cutfile = new TFile("cut/cut_Q1vsEdiscri_neutron_CHINUhg.root","update");
  //TFile* cutfile = new TFile("cut/cut_Q1vsEdiscri_neutron_B3lg.root","update");
  TFile* cutfile = new TFile("cut/cut_Q1vsEdiscri_neutron_B3hg.root","update");

  histofile->ls();

  TCanvas* c1 = new TCanvas("c1","c1",1200,1200);
  c1->cd(); gPad->SetLogx(1);gPad->SetLogy(1);
  ...
  for(int det_number=1; det_number<=B3_nDets; det_number++){
    ...
    cutg->SetName(Form("cut_Q1vsE_neutron_%i",det_number));
    cutfile->cd();
    cutg->Write();
  }
  histofile->Close();
